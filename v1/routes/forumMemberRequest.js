...
router.post("/", authentication, async function (req, res, next) {
    ...
    try {
        ...
        insertionData.userId = await services.addUserCityMapping(
            userId,
            cityId
        );
    } catch (err) {
...
router.delete("/:id", authentication, async function (req, res, next) {
    ...
    try {
        ...
        const response = await database.get(
            tables.FORUM_REQUEST,
            { id: memberRequestId, forumId: forum.id },
            null,
            cityId
        );
        ...
        const cityuser = await database.get(
            tables.USER_CITYUSER_MAPPING_TABLE,
            { userId, cityId }
        );
        if (
            !cityuser.rows ||
            forumRequest.userId !== cityuser.rows[0].cityUserId
        ) {
            return next(
                new AppError(
                    `You dont have the permission to delete this request! `,
                    403
                )
            );
        } else {
    ...
    try {
        ...
        if (!forumMember.isAdmin) {
            ...
            if (payload.accept) {
                ...
                const cityTransaction =
                    await database.createTransaction(cityId);
                try {
