...
router.get("/", authentication, async function (req, res, next) {
    ...
    try {
        ...
        const response = await services.getForumMessages(
            city.id,
            forum,
            lastMessageId,
            isReversed,
...
router.post("/", authentication, async function (req, res, next) {
    ...
    if (
        !payload.messageType ||
        Object.values(messageTypes).indexOf(payload.messageType) === -1
    ) {
        return next(
            new AppError(`Invalid payload: messageType is required`, 400)
        );
    }
    if (!payload.message) {
    ...
    if (!payload.groupKeyVersion) {
        if (forum.isPrivate) {
            return next(
                new AppError(
                    `Invalid payload: groupKeyVersion is required for private forums`,
                    400
                )
            );
        }
        ...
        if (!response.rows.length) {
            ...
            response.rows[0].groupKeyVersion !== payload.groupKeyVersion
        ) {
            return next(
                new AppError(
                    `Invalid payload: groupKeyVersion is not the latest`,
                    400
                )
            );
        }
        ...
        try {
            ...
            if (process.env.FIREBASE_PRIVATE) {
                await sendPushNotificationToAll(
                    `groupChat_city_${cityId}_forum_${forumId}`,
                    forum.forumName,
