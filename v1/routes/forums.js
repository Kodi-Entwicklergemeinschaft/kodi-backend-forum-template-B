...
router.get("/", async function (req, res, next) {
    ...
    try {
        ...
        const response = await database.get(
            tables.FORUMS,
            null,
            `id, forumName, createdAt, description, image, isPrivate, ${city.id} as cityId`,
            city.id,
            pageNo,
            pageSize
        );
...
router.post("/", authentication, async function (req, res, next) {
    ...
    try {
        ...
        const cityUserId = await services.addUserCityMapping(
            req.userId,
            req.cityId
        );
...
router.delete(
    ...
    async function (req, res, next) {
        ...
        try {
            const city = await services.getCity(cityId);
            await services.getForum(forumId, city.id);
            await services.getForumMember(userId, forumId, cityId);
            const imagePath = `user_${userId}/city_${cityId}_forum_${forumId}_image`;
