...
function encodeURIWithSafe(str, safe) {
    ...
    for (let i = 0; i < str.length; i++) {
        ret.push(
            safe.indexOf(str[i]) >= 0 ? str[i] : encodeURIComponent(str[i])
        );
    }
...
function v2Auth(ak, sk, opt, log, methodName) {
    ...
    if (opt["urlPath"] !== "") {
        ...
        for (let i in arrPath) {
            ...
            if (
                allowedResourceParameterNames.indexOf(
                    listvar[0].toLowerCase()
                ) >= 0
            ) {
                let sep = urlPath === "" ? "?" : "&";
                urlPath += sep;
                if (listvar.length === 2) {
                    urlPath +=
                        listvar[0] + "=" + decodeURIComponent(listvar[1]);
                } else {
                    urlPath += strOpt;
                }
            }
    ...
    opt["headers"]["authorization"] =
        ...
        ak +
        ":" +
        crypto.createHmac("sha1", sk).update(stringToSign).digest("base64");
    return opt;
}
    ...
    for (let key in obj) {
        if (obj[key]["location"] === "header") {
            ...
            if (obj[key]["type"] === "object") {
                opt["InterfaceResult"][key] = parseObjectFromHeaders(
                    sentAs,
                    headers
                );
            } else {
...
function getRequest(
    ...
    log.runLog(
        "info",
        methodName,
        "get response start, statusCode:" + serverback.statusCode
    );
    log.runLog(
        "debug",
        methodName,
        ...
        if (filePath) {
            ...
            stream.on("error", function (err) {
                ...
                log.runLog(
                    "error",
                    methodName,
                    "get response stream error [" + headerStr + "]"
                );
                log.interfaceLog(
                    "error",
                    methodName,
        ...
        if (result !== "") {
            ...
            if ("data" in xmlMeta) {
                if ("xsiNamespace" in xmlMeta["data"]) {
                    xml +=
                        '  xmlns:xsi="' + xmlMeta["data"]["xsiNamespace"] + '"';
                }
                if ("xsiType" in xmlMeta["data"]) {
                    xml +=
                        '  xsi:type="' +
                        mXml[key][xmlMeta["data"]["xsiType"]] +
                        '"';
                }
            }
            xml += ">";
    ...
    if (opt[key] === undefined) {
        if (model[key]["type"] === "object") {
            opt[key] = model[key]["parameters"]
                ? jsonToObject(model[key]["parameters"], null, null)
                : {};
        } else if (model[key]["type"] === "array") {
            opt[key] = [];
        } else {
...
function v4Auth(region, ak, sk, opt, log, methodName) {
    ...
    var credenttial =
        ak + "/" + shortDate + "/" + region + "/" + "s3" + "/" + "aws4_request";
...
Utils.prototype.initFactory = function (
    ...
    this.port =
        port === undefined ? (this.is_secure ? 443 : 80) : parseInt(port);
    ...
    if (long_conn_param !== undefined && Number(long_conn_param) >= 0) {
        this.httpAgent = new this.http.Agent({
            keepAlive: true,
            keepAliveMsecs: Number(long_conn_param) * 1000
        });
        this.httpsAgent = new this.https.Agent({
            keepAlive: true,
            keepAliveMsecs: Number(long_conn_param) * 1000
        });
    }
...
Utils.prototype.doSend = function (
    req,
    srcFile,
    initPos,
    totalCount,
    ...
    fs.open(srcFile, "r", function (err, fd) {
        ...
        var _doSend = function (sendPosition, sendCount) {
            ...
            var sendCountOnce =
                totalCount - sendCount > buffer.length
                    ? buffer.length
...
Utils.prototype.makeRequest = function (methodName, opt, bc) {
    ...
    if ("xml" in opt) {
        ...
        if (!(body instanceof streamLib.Readable)) {
            ...
            opt["headers"]["content-length"] =
                body.length === 0 ? "0" : String(body.length);
        }
    }
    ...
    opt =
        this.signature.toLowerCase() === "v4"
            ? v4Auth(this.region, ak, sk, opt, log, methodName)
            : v2Auth(ak, sk, opt, log, methodName);
    ...
    var reopt = {
        ...
        path: path,
        ca:
            !this.ssl_verify || !fs.existsSync(String(this.ssl_verify))
                ? null
                : fs.readFileSync(String(this.ssl_verify)),
        ...
        if (readable) {
            if ("ContentLength" in opt) {
                ...
                body.on("data", function (data) {
                    ...
                    if (contentLength - writeCount > data.length) {
                        ...
                        req.write(
                            Buffer.from(data, 0, contentLength - writeCount)
                        );
            ...
            body.on("error", function (err) {
                ...
                log.runLog(
                    "error",
                    methodName,
                    "read file to send error [" + headerStr + "]"
                );
                log.interfaceLog(
                    "error",
                    methodName,
            ...
            if (meta["location"] === "uri") {
                ...
                if (meta["type"] === "object") {
                    if ("x-amz-meta-" === sentAs) {
                        for (let item in param[key]) {
                            let iKey =
                                item.toLowerCase().indexOf(sentAs) === 0
                                    ? item.toLowerCase()
                                    : sentAs + item.toLowerCase();
                            exheaders[iKey.trim()] = encodeURIWithSafe(
                                param[key][item],
                                " ,:?/+=%"
                            );
                        }
                ...
                if (meta["type"] === "number") {
                    ...
                    urlPath +=
                        ...
                        encodeURIWithSafe(sentAs, ",:?&%") +
                        "=" +
                        encodeURIWithSafe(String(param[key]), ",:?&%");
                }
    ...
    if ("srcFile" in opt) {
        ...
        if (!("content-length" in opt["headers"])) {
            if ("PartSize" in opt || "Offset" in opt) {
                ...
                offset =
                    offset && offset >= 0 && offset < fileSize ? offset : 0;
                partSize =
...
Utils.prototype.sendRequest = function (funcName, opt, backcall, count) {
    ...
    this.makeRequest(funcName, opt, function (err, msg) {
        if (
            err &&
            msg !== "SELF_SIGNED_CERT_IN_CHAIN" &&
            msg !== "DEPTH_ZERO_SELF_SIGNED_CERT" &&
            count <= that.max_retry_count
        ) {
...
Utils.prototype.fileMD5Partly = function (filePath, offset, partSize) {
    ...
    fs.stat(filePath, function (err, stats) {
        ...
        if (!stats.isFile()) {
            eventEmitter.emit(
                "error",
                "the file [" + filePath + "] is not a valid file!"
            );
            return;
        }
        ...
        fs.open(filePath, "r", function (err, fd) {
            ...
            var doRead = function (readPosition, readCount) {
                ...
                var readCountOnce =
                    partSize - readCount > buffer.length
                        ? buffer.length
                        : partSize - readCount;
...
Utils.prototype.createV2SignedUrlSync = function (param) {
    ...
    var specialParam = param["SpecialParam"]
        ? String(param["SpecialParam"])
        : null;
    ...
    for (let key in headers) {
        if (
            key.length > "x-amz-".length &&
            key.slice(0, "x-amz-".length) === "x-amz-"
        ) {
            amz[i++] = key;
        }
    }
...
Utils.prototype.createV4PostSignatureSync = function (param) {
    ...
    var signature = createV4Signature(
        shortDate,
        this.secretAccessKey,
        this.region,
        policy
