...
ObsClient.prototype.setBucketVersioningConfiguration = function (
    param,
    callback
) {
...
ObsClient.prototype.getBucketVersioningConfiguration = function (
    param,
    callback
) {
...
ObsClient.prototype.setBucketLifecycleConfiguration = function (
    param,
    callback
) {
...
ObsClient.prototype.getBucketLifecycleConfiguration = function (
    param,
    callback
) {
...
ObsClient.prototype.deleteBucketLifecycleConfiguration = function (
    param,
    callback
) {
...
ObsClient.prototype.deleteBucketWebsiteConfiguration = function (
    param,
    callback
) {
...
ObsClient.prototype.putObject = function (param, callback) {
    ...
    if (!("ContentType" in param)) {
        ...
        if (!param["ContentType"] && "SourceFile" in param) {
            param["ContentType"] =
                this.util.mimeTypes[
                    param["SourceFile"].substring(
                        param["SourceFile"].lastIndexOf(".") + 1
                    )
                ];
        }
    }
...
ObsClient.prototype.uploadFile = function (param, callback) {
    ...
    var _callback = function (code, message, result) {
        ...
        if (typeof code === "string") {
            ...
            callback(
                code
                    ? code.CommonMsg.Code + ":" + code.CommonMsg.Message
                    : null,
                result
            );
        }
    ...
    _log.runLog(
        "debug",
        funcName,
        "Begin to uploadFile to OBS from file:" + uploadFile
    );
    ...
    var calculateUploadCheckpointMD5 = function (uploadCheckpoint) {
        ...
        if (
            ...
            uploadCheckpoint.partsMeta instanceof Object
        ) {
    ...
    var writeCheckpointFileSync = function (checkpointFile, uploadCheckpoint) {
        try {
            ...
            _log.runLog(
                "debug",
                funcName,
                "Write checkpoint file failed and abort upload, uploadId " +
        ...
        if (uploadCheckpoint && uploadCheckpoint.uploadId) {
            obsClient.abortMultipartUpload(
                ...
                function (e, r) {
                    if (
                        !e &&
                        (r.CommonMsg.Status < 300 || r.CommonMsg.Status === 404)
                    ) {
                        if (checkpointFile) {
                            fs.unlink(checkpointFile, function () {});
                        }
    ...
    eventEmitter.on("start to uploadFile", function (uploadCheckpoint) {
        ...
        var startToUploadFile = function (uploadId) {
                    ...
                    if (!hasError) {
                        ...
                        for (let partNumber in uploadCheckpoint.partsMeta) {
                            parts.push({
                                ...
                                ETag: uploadCheckpoint.partsMeta[partNumber]
                                    .etag
                            });
                        }
                        ...
                        if (checkpointFile) {
                            _callback(
                                "IncompletedUpload",
                                "UploadFile finished with error, you can retry with the same parameters"
                            );
                        } else {
                            abortRequest(
                                uploadCheckpoint,
                                checkpointFile,
                                "UploadFileFailed",
                                "UploadFile finished with error!"
                            );
                        }
                    }
                }
            ...
            eventEmitter.on(
                ...
                function (currentGroupIndex, groupCount, taskNum) {
                    ...
                    while (start < end) {
                        ...
                        if (
                            uploadCheckpoint.partsMeta[partNumber] &&
                            uploadCheckpoint.partsMeta[partNumber].isCompleted
                        ) {
                            finishedCount++;
                            finishedCountGroup++;
                            let _taskNum =
                                currentGroupIndex === groupCount
                                    ? uploadCheckpoint.partCount -
                                      (currentGroupIndex - 1) * taskNum
                                    : taskNum;
                            if (
                                finishedCountGroup === _taskNum &&
                                finishedCount < uploadCheckpoint.partCount
                            ) {
                                eventEmitter.emit(
                                    "doUpload",
                                    currentGroupIndex + 1,
                            ...
                            let currPartSize =
                                start + 1 === uploadCheckpoint.partCount
                                    ? uploadCheckpoint.uploadFileStat.fileSize -
                                      offset
                                    : uploadCheckpoint.partSize;
                            obsClient.uploadPart(
                                ...
                                function (err, result) {
                                    ...
                                    uploadCheckpoint.md5 =
                                        calculateUploadCheckpointMD5(
                                            uploadCheckpoint
                                        );
                                    if (
                                        checkpointFile &&
            ...
            let groupCount =
                uploadCheckpoint.partCount % taskNum === 0
                    ? uploadCheckpoint.partCount / taskNum
                    : Math.floor(uploadCheckpoint.partCount / taskNum) + 1;
            eventEmitter.emit("doUpload", 1, groupCount, taskNum);
        };

        let uploadId = uploadCheckpoint.uploadId;
        if (!uploadId) {
            let contentType = uploadCheckpoint.contentType
                ? uploadCheckpoint.contentType
                : obsClient.util.mimeTypes[
            ...
            obsClient.initiateMultipartUpload(
                {
                    ...
                    WebsiteRedirectLocation:
                        uploadCheckpoint.websiteRedirectLocation,
                ...
                function (err, result) {
                    ...
                    uploadCheckpoint.md5 =
                        calculateUploadCheckpointMD5(uploadCheckpoint);
                    _log.runLog(
                        "debug",
                        funcName,
                        "Claim a new upload id " + uploadId
                    );
                    if (
                        checkpointFile &&
                        !writeCheckpointFileSync(
    ...
    uploadCheckpoint.uploadFileStat = {
        fileSize: fileSize,
        lastModified: fstat.mtime.toUTCString()
    };

    if (checkpointFile) {
        if (!fs.existsSync(checkpointFile)) {
            // call uploadFile first time
            eventEmitter.on("start to writeFile", function () {
                fs.writeFile(
                    checkpointFile,
                    JSON.stringify(uploadCheckpoint),
                    function (err) {
                        if (err) {
            ...
            fs.readFile(checkpointFile, function (err, data) {
                ...
                if (
                    !uploadCheckpoint ||
                    uploadCheckpoint.bucketName !== bucket ||
                    uploadCheckpoint.objectKey !== key ||
                    uploadCheckpoint.uploadFile !== uploadFile ||
                    !uploadCheckpoint.uploadFileStat ||
                    uploadCheckpoint.uploadFileStat.fileSize !== fstat.size ||
                    uploadCheckpoint.uploadFileStat.lastModified !==
                        fstat.mtime.toUTCString() ||
                    uploadCheckpoint.md5 !==
                        calculateUploadCheckpointMD5(uploadCheckpoint)
                ) {
                    abortRequest(
                        uploadCheckpoint,
                        checkpointFile,
                        "CheckpointFileInvalid",
                        "CheckpointFile is invalid!"
                    );
                    return;
                }

                if (uploadCheckpoint.uploadFileStat.checkSum) {
                    obsClient.util.fileMD5(
                        uploadFile,
                        function (err, checkSum) {
                            if (
                                err ||
                                checkSum !==
                                    uploadCheckpoint.uploadFileStat.checkSum
                            ) {
                                abortRequest(
                                    uploadCheckpoint,
                                    checkpointFile,
...
ObsClient.prototype.downloadFile = function (param, callback) {
    ...
    if (!param.DownloadFile) {
        ...
        _log.runLog(
            "info",
            funcName,
            "DownloadFile is not set, will put file to path:" + downloadFile
        );
    } else {
        ...
        function (err, result) {
            ...
            let calculateDownloadCheckpointMD5 = function (downloadCheckpoint) {
                ...
                if (
                    ...
                    downloadCheckpoint.partsMeta instanceof Object
                ) {
            ...
            let writeCheckpointFileSync = function (
                checkpointFile,
                downloadCheckpoint
            ) {
                try {
                    if (fs.existsSync(downloadTempFile)) {
                        ...
                        downloadCheckpoint.tempFileStatus.fileSize =
                            tempFileStat.size;
                        downloadCheckpoint.tempFileStatus.lastModified =
                            tempFileStat.mtime.toUTCString();
                    }
                    fs.writeFileSync(
                        checkpointFile,
                        JSON.stringify(downloadCheckpoint)
                    );
                    return true;
                } catch (e) {
                    fs.unlink(checkpointFile, function () {});
            ...
            eventEmitter.on(
                ...
                function (downloadCheckpoint) {
                    ...
                    _log.runLog(
                        "debug",
                        funcName,
                        "Begin to downloadFile from OBS to:" + downloadFile
                    );
                    ...
                    fs.open(downloadTempFile, "a", function (err, fd) {
                        ...
                        let completedDownload = function (result) {
                            if (
                                finishedCount === downloadCheckpoint.partCount
                            ) {
                                try {
                                    fs.closeSync(fd);
                                } catch (e) {
                                ...
                                if (!hasError) {
                                        ...
                                        function (err) {
                                            if (err) {
                                                if (checkpointFile) {
                                                    ...
                                                    _callback(
                                                        "IncompletedDownload",
                                                        "DownloadFile finished but rename temp file error, you can retry with the same parameters, err:" +
                                                            err
                                                    );
                                                } else {
                                                    fs.unlink(
                                                    ...
                                                    _callback(
                                                        "DownloadFileFailed",
                                                        "DownloadFile finished but rename temp file error"
                                                    );
                                                }
                                                return;
                                            }
                                    ...
                                    if (checkpointFile) {
                                        ...
                                        _callback(
                                            "IncompletedDownload",
                                            "DownloadFile finished with error, you can retry with the same parameters"
                                        );
                                    } else {
                                        fs.unlink(
                                            downloadTempFile,
                                        ...
                                        _callback(
                                            "DownloadFileFailed",
                                            "DownloadFile finished with error"
                                        );
                                    }
                                }
                            }
                        ...
                        eventEmitter.on(
                            ...
                            function (currentGroupIndex, groupCount, taskNum) {
                                ...
                                while (start < end) {
                                    ...
                                    let i = start + 1;
                                    if (
                                        downloadCheckpoint.partsMeta[i] &&
                                        downloadCheckpoint.partsMeta[i]
                                            .isCompleted
                                    ) {
                                        finishedCount++;
                                        ...
                                        let startPos =
                                            (i - 1) *
                                            downloadCheckpoint.partSize;
                                        let endPos =
                                            i === downloadCheckpoint.partCount
                                                ? downloadCheckpoint
                                                      .objectStatus.objectSize -
                                                  1
                                                : i *
                                                      downloadCheckpoint.partSize -
                                                  1;
                                        obsClient.getObject(
                                            {
                                                ...
                                                SaveAsStream: true,
                                                IfMatch:
                                                    downloadCheckpoint.ifMatch,
                                                IfModifiedSince:
                                                    downloadCheckpoint.ifModifiedSince,
                                                IfNoneMatch:
                                                    downloadCheckpoint.ifNoneMatch,
                                                IfUnmodifiedSince:
                                                    downloadCheckpoint.ifUnmodifiedSince,
                                            ...
                                            function (err, result) {
                                                ...
                                                if (
                                                    ...
                                                    downloadCheckpoint.md5 =
                                                        calculateDownloadCheckpointMD5(
                                                            downloadCheckpoint
                                                        );
                                                    if (
                                                        checkpointFile &&
                                                    ...
                                                    result.InterfaceResult.Content.on(
                                                        "data",
                                                        function (data) {
                                                            fs.write(
                                                                fd,
                        ...
                        let groupCount =
                            downloadCheckpoint.partCount % taskNum === 0
                                ? downloadCheckpoint.partCount / taskNum
                                : Math.floor(
                                      downloadCheckpoint.partCount / taskNum
                                  ) + 1;
            ...
            partSize = isNaN(partSize)
                ...
                : partSize > 0
                  ...
                  : 5 * 1024 * 1024;
            let partCount =
                objectSize % partSize === 0
                    ? Math.floor(objectSize / partSize)
            ...
            downloadCheckpoint.md5 =
                calculateDownloadCheckpointMD5(downloadCheckpoint);
            downloadCheckpoint.objectStatus = {
                objectSize: objectSize,
                lastModified: objestStat.InterfaceResult.LastModified,
                etag: objestStat.InterfaceResult.ETag
            };

            if (checkpointFile) {
                if (!fs.existsSync(checkpointFile)) {
                    // call downloadFile first time
                    if (fs.existsSync(downloadTempFile)) {
                        try {
                    ...
                    fs.readFile(checkpointFile, function (err, data) {
                        ...
                        if (
                            !downloadCheckpoint ||
                            downloadCheckpoint.bucketName !== bucket ||
                            downloadCheckpoint.objectKey !== key ||
                            downloadCheckpoint.downloadFile !== downloadFile ||
                            !downloadCheckpoint.objectStatus ||
                            downloadCheckpoint.objectStatus.objectSize !==
                                objectSize ||
                            downloadCheckpoint.objectStatus.lastModified !==
                                objestStat.InterfaceResult.LastModified ||
                            downloadCheckpoint.objectStatus.etag !==
                                objestStat.InterfaceResult.ETag ||
                            !verifyTempFileStatus(
                                downloadCheckpoint.tempFileStatus
                            ) ||
                            downloadCheckpoint.md5 !==
                                calculateDownloadCheckpointMD5(
