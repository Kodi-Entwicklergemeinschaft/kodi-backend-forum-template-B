...
const getForumMemberStatus = async (forumIds, cityId, userId) => {
    try {
        ...
        const forummember = await database.get(tables.FORUM_MEMBERS, { forumId: forumIds, userId }, ["forumId", "userId"], cityId);
        if (forummember.rows) {
            ...
            result = forummember.rows.map((member) => {
                ...
                return { "forumId": +member.forumId, "statusId": status.Accepted };
            }
            );

        }
        if (filteredForumIds.length > 0) {
            const response = await database.get(tables.FORUM_REQUEST, { forumId: filteredForumIds, userId }, ["forumId", "statusId"], cityId);
            const temp = response.rows.map((row) => row.forumId);
            noResponseForumsIDs = filteredForumIds.filter((forumId) => !temp.includes(+forumId)).map((id) => { return { "forumId": +id, "statusId": 0 } });
...
const getCityUser = async (userId, cityId, throwErrorIfUserNotMember = true) => {
    try {
        const response = await database.get(
...
const getForumMember = async (userId, forumId, cityId, throwErrorIfUserNotMember = true, cityUser = false) => {
    try {
...
const getForumMessages = async (cityId, forumData, lastMessageId, isReversed, pageNo, pageSize) => {
    try {
        let query;
...
const addUserCityMapping = async (userId, cityId) => {
    try {
        ...
        let cityUserId = 0;
        if (!response.rows || response.rows.length === 0) {
            const query = "id, username, firstname, lastname, email, phoneNumber, image, description, website, roleId";
            const userResponse = await database.get(tables.USER_TABLE, { id: userId }, query);
...
module.exports = { getCity, getForum, getCityUser, getForumMember, getForumPost, getForumMemberStatus, addUserCityMapping, getForumMessages, getUser };
